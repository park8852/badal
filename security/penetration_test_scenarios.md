...
### G-02. SQL Injection 방어 동작 확인
**점검 항목**: SQL Injection  
**진단 결과**: 양호  
**취약점 개요**: 데이터베이스 조회·수정을 수행하는 API가 사용자 입력을 안전하게 처리하고 있는지 검증  
...
**조치 방법**  
서버는 `PreparedStatement`/ORM 바인딩을 사용하고 입력값을 화이트리스트 및 길이 제한으로 검증한다. 앱은 쿼리 문자열 직접 조합을 지양하고, 에러 응답에 SQL 정보를 포함하지 않도록 한다.

**재현 절차**  
Step 1) Burp Suite Intercept ON 상태에서 로그인 API(`POST http://localhost:8080/api/member/login`)를 캡처한 뒤, `userid` 값에 `' OR '1'='1` 페이로드를 주입한다.  
![G02-Step1](./evidence/g02_step1_payload.png "Burp Suite에서 SQL Injection 페이로드를 주입하는 화면")

Step 2) 요청을 전송하면 서버는 `responseType`을 `ERROR`로 반환하고, 메시지 “로그인 실패”와 함께 인증이 거부된다. 응답에는 SQL 에러 메시지나 스택 트레이스가 포함되지 않는다.  
![G02-Step2](./evidence/g02_step2_response.png "서버가 에러 메시지로 차단한 결과 화면")

Step 3) 동일 페이로드로 주문조회 API(`GET http://localhost:8080/api/order/list`)에 쿼리 문자열(예: `?id=' UNION SELECT ...`)을 삽입해도 HTTP 400 응답과 함께 “잘못된 요청입니다” 메시지가 반환되며, 응답 본문에 데이터베이스 구조가 노출되지 않는다.  
![G02-Step3](./evidence/g02_step3_order.png "주문 조회 API가 비정상 쿼리를 차단한 캡처")

**진단 세부 사항**  
- Burp Suite 패킷과 서버 로그를 확인한 결과, SQL 에러 로그 또는 예외 스택이 노출되지 않았음.  
- `MemberRepository`와 주요 DAO에서 `JdbcTemplate`의 파라미터 바인딩을 사용하고 있어, SQL 구문이 문자열로 직접 결합되지 않음을 확인함.  
- 추가 검증이 필요한 엔드포인트는 없으며, 현재 기준으로 **양호** 판정.
